...
export default class WaterbodyInfo extends React.Component {
  ...
  {measurementInfo && (
    <div className="column-container">
      ...
      <Info key={2} label="Mission name" value={thousands_separators(`${measurementInfo.sensor_type}`)}   />
      ...
      {outline && (<Info key={5} label="Water Volume<span style={{ verticalAlign: "super" }}>abc</sup>" value={thousands_separators(`${measurementInfo.volume}`)}   />)}
    </div>
  )}
</div>
